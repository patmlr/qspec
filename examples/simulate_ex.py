...
def example(n: Union[set, int] = None):
    ...
    Example 0: Interaction between a singly-charged calcium ion and two lasers.
    ...
    if 1 in n:
        ...
        In example 1, the master equation is solved for 43Ca+.
        """
        f_sp = 755222766  # The 4s -> 4p 2P1/2 transition frequency.
        f_dp = 346000235  # The 3d 2D3/2 -> 4p 2P1/2 transition frequency.
        ...
        inter = sim.Interaction(atom=ca43, lasers=[laser_sp, laser_dp], delta_max=400.)
        # inter.resonance_info()  # Print the detuning of the lasers from the considered transitions.
        ...
        inter.master(t=0.2)  # Solve the master equation for 0.2 us, assuming equal population in all s-states.
        # (Here the s-states are the first in the list and they all have the same label.)
    ...
    if 2 in n:
        ...
        inter.master(t=10, x_scale='log')  # Solve the master equation for 10 us and plot with logarithmic scaling.

        delta = np.linspace(-2., 2., 101)
        y0 = li7.get_y0(['s3', 's5'])

        # Solve for the first 10 us (these are 10 000 time steps times 101 detunings)
        # inter.spectrum(10, delta, m=1, y0=y0, solver='master', x_scale='log')  # with the master equation ...
        # inter.spectrum(10, delta, m=1, y0=y0, solver='rates', x_scale='log')  # ... and the rate equations.
