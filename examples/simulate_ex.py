...
def example(n: int | set[int] | None = None) -> None:
    ...
    if 0 in n:
        ...
        times = np.linspace(0.0, 0.5, 1001)  # Integration times.
        ...
        plt.ylim(0, 1)
        plt.xlabel(f"f - {laser_sp.freq} (MHz)")
        plt.ylabel(f"state population after {times[i_t]} us")
        plt.legend()
        plt.show()
    ...
    if 1 in n:
        ...
        _h = inter.hamiltonian(0.0, [5.0], 1, [0.0])
        ...
        y = sim.density_matrix_diagonal(results)[:, :, -1]
        ...
        plt.xlabel(f"f - {laser_sp.freq} (MHz)")
        plt.ylabel(f"state population after {times[-1]} us")
        plt.legend()
        plt.show()
    ...
    if 2 in n:
        ...
        inter = sim.Interaction(atom=ca43, lasers=[laser_sp], delta_max=400.0)
        ...
        times = [0.0, 0.2]  # Integration time in us.
        ...
        for f in qs.get_f(i, 2.5):
            plt.plot(delta, np.sum(results[:, ca43.get_state_indexes("d5", f), -1], axis=1), label=f"F={f}")
        plt.xlabel(f"f - {laser_sp.freq} (MHz)")
        plt.ylabel(f"d5-state population after {times[-1]} us")
        plt.legend()
        plt.show()
    ...
    if 3 in n:
        ...
        laser_r = sim.Laser(freq=f - 13566.0, polarization=pol_r, intensity=i_r)  # red laser

        print(f"Saturation s(blue): {qs.saturation(i_b, f, a)}")
        print(f"Saturation s(red): {qs.saturation(i_r, f, a)}")
        ...
        y = sim.density_matrix_diagonal(y)[0]

        plt.xscale("log")
        for s in ["s3", "s5", "p"]:
            plt.plot(t, np.sum(y[li7.get_state_indexes(s)], axis=0), label=s)
        plt.xlabel("time (us)")
        plt.ylabel("state population")
        plt.legend()
        plt.show()
    ...
    if 4 in n:
        ...
        env = sim.Environment(B=[0.0, 0.0, 1e-6])
        inter = sim.Interaction(
            atom=he,
            lasers=[
                laser_sp,
            ],
        ...
        plt.xlabel(f"f - {laser_sp.freq} (MHz)")
        plt.ylabel(f"scattering rate after {times[-1]} us (MHz)")
        plt.legend()
        plt.show()
    ...
    if 5 in n:
        ...
        inter = sim.Interaction(atom=he, lasers=[laser_0, laser_1], delta_max=500.0)
    ...
    if 7 in n:
        ...
        s1 = sim.gen_electronic_state(0.0, parity="even", j=0, i=0, ls=[0, 0], label="s1")
        ...
        y0[atom.get_state_indexes("s3")] = 1.0

        t = np.linspace(0.0, 100000.0, 101)
    ...
    if 8 in n:
        ...
        states = s + d3
        # states = s + [d3[-1]]
        decay_map = sim.DecayMap(labels=[("s", "d3"), ("s", "d5")], a=[{"m1": 0.01 * a, "e2": a}, {"e2": a}], k_max=2)
    ...
    if 9 in n:
        ...
        g = sim.State(0.0, parity="e", j=jg, i=i, f=jg + i, m=jg + i, hyper_const=g_hyper, label="g")
        ...
        laser_eg = sim.Laser(freq=f_eg, polarization=pol_eg, intensity=intensity, k=[0.0, 1.0, 0.0])
        ...
        r0 = atom.scattering_rate(rho[0, :, :, -1], theta=np.pi / 2, phi=0.0, x_vec="-", axis=0)
        print(f"r0: {r0[0]} MHz")
