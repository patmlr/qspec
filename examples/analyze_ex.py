...
def example(n: int | set[int] | None = None) -> None:
    ...
    if 0 in n:
        ...
        dxy = np.array(
            [
                rng.multivariate_normal(
                    mean=[0, 0],
                    cov=[[sigma_x**2, ci * sigma_x * sigma_y], [ci * sigma_x * sigma_y, sigma_y**2]],
                    size=1,
                )
                for ci in c
            ]
        ...
        popt_cf, _pcov_cf = qs.curve_fit(qs.straight, x, y, sigma=sy, report=True)
        # Do a standard fit with y-axis uncertainties.

        popt_of, _pcov_of = qs.odr_fit(qs.straight, x, y, sigma_x=sx, sigma_y=sy, report=True)
        # Do an orthogonal-distance-regression fit with x- and y-axis uncertainties.

        popt_yf, _pcov_yf = qs.york_fit(x, y, sigma_x=sx, sigma_y=sy, corr=c, report=True)
        ...
        popt_lf, _pcov_lf = qs.linear_fit(x, y, sigma_x=sx, sigma_y=sy, corr=c, report=True)
    ...
    if 1 in n:
        ...
        mean = np.array([rng.normal(scale=sigma[0]) + t * s for t in range(size)])
        ...
        popt, pcov = qs.linear_nd_monte_carlo(
            mean,
            cov=cov,
            n_samples=100000,
            method="cpp",
        ...
        s_fit = qs.straight_std(
            x_fit - popt[0], np.sqrt(pcov[1, 1]), np.sqrt(pcov[4, 4]), pcov[1, 4] / np.sqrt(pcov[1, 1] * pcov[4, 4])
        )
        ...
        s_fit = qs.straight_std(
            x_fit, np.sqrt(pcov[2, 2]), np.sqrt(pcov[5, 5]), pcov[2, 5] / np.sqrt(pcov[2, 2] * pcov[5, 5])
        )
    ...
    if 2 in n:
        ...
        vals = [
            ...
            * 5,  # Fill the unkown axes.  # 43Ca
            [
                (755223614.656, 0.098),
                (761905862.618, 0.092),  # 44Ca
                (345995736.343, 0.298),
                (352677984.496, 0.258),
                (350858392.304, 0.261),
            ],
            ...
            * 5,  # Fill the unkown axes.  # 46Ca
            [
                (755224471.117, 0.102),
                (761906720.109, 0.114),  # 48Ca
                (345991937.638, 0.284),
                (352674186.532, 0.368),
                (350854600.002, 0.215),
            ],
        ...
        x, cov, _cov_stat = king.get_unmodified(a_unknown, a_unknown_ref, y, axis=1, show=True)
