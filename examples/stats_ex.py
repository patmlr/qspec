...
def example(n: int | set[int] | None = None) -> None:
    ...
    def sin(_x, _f, _a, _b, _c):  # noqa: ANN001, ANN202
    ...
    if 0 in n:
        ...
        print(f"5 * f = {5 * f}")
        print(f"f ** 2 = {f**2}")
        print(f"f + 2 * f = {f + 2 * f}")  # If formulae are getting to long or contain non-standard operators,
        # it is more efficient/necessary to use propagate and define the function (see 'sin' above):

        x, a, b, c = 0.2, 3, 10, 5
        print(f"\n1. sin = {qs.propagate(sin, [x, f, a, b, c])}")
    ...
    if 1 in n:
        ...
        popt, pcov, *_ = qs.curve_fit(sin, x, y, sigma=sigma, absolute_sigma=True, p0=np.array(p0)[:, 0])
        y_fit = sin(x, *popt)
