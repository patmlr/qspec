...
def freq_to_cm1(f: array_like) -> ndarray:
    r"""
...
def wavelength_to_cm1(lam: array_like) -> ndarray:
    r"""
...
def hyper_zeeman_ij(
    ...
    $$
    H = \sum\limits_{k=1}^{3} \left[\mathbf{T}_I^{(k)}\otimes\mathbf{T}_J^{(k)}\right]^{(0)}
     - \vec{\mu}_F\cdot\vec{\mathcal{B}}.
    $$
