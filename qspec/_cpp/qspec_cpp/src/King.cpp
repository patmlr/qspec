...
CollinearResult collinear(std::vector<VectorXd> x, std::vector<MatrixXd> cov, size_t n, size_t n_max, unsigned int seed, bool report)
{
	...
	std::vector<MultivariateNormal> mvn_vec(size);
	for (size_t i = 0; i < size; ++i) mvn_vec.at(i) = MultivariateNormal(x.at(i), cov.at(i));
	...
	while (n_accepted < n && n_samples < n_max)
	{
		...
		VectorXd p0 = mvn_vec.front().rvs(gen);
		VectorXd p1 = mvn_vec.back().rvs(gen);
		VectorXd dr = p1 - p0;
		dr /= dr.norm();
		...
		{
			MultivariateNormal& _mvn = mvn_vec.at(i + 1);
