...
void Environment::set_E(double Eabs)
{
	E = Eabs;
}

void Environment::set_E(Vector3d Evec)
{
	E = Evec.norm();
	if (E == 0)
	{
...
void Environment::set_B(double Babs)
{
	B = Babs;
}

void Environment::set_B(Vector3d Bvec)
{
	B = Bvec.norm();
	if (B == 0)
	{
				 ...
				 double* _hyper_const, double _gj, double _gi, std::string _label)
{
	...
	double f_min = std::max(abs(m), abs(i - j));
	double f_max = i + j;
...
{
	return fmt::format("{}({}, {}, {}, {})", label, half_int_to_str(j), half_int_to_str(i),
		half_int_to_str(f), half_int_to_str(m));
}
	...
	for (size_t k = 1; k <= decays->get_k_em_max(); ++k)
	{
		...
		for (size_t i = 1; i < size; ++i)
		{
			for (size_t j = 0; j < i; ++j)
			{
				...
				double fi_min = std::max(abs(states[_i]->get_m()), abs(states[_i]->get_i() - states[_i]->get_j()));
				double fi_max = states[_i]->get_i() + states[_i]->get_j();

				double fj_min = std::max(abs(states[_j]->get_m()), abs(states[_j]->get_i() - states[_j]->get_j()));
				...
				// printf("%s\n", fmt::format("ak({}, {}): {:.0e}", _i, _j, ak).c_str());
...
void Atom::gen_frequencies(Environment* _env)
{
	...
	for (size_t k = 0; k < size; ++k)
	{
		...
		State& s = *states.at(k);
		double f_min = std::max(abs(s.get_m()), abs(s.get_i() - s.get_j()));
	...
	std::vector<std::vector<MatrixXcd>>& qk, std::vector<size_t>& i, std::vector<size_t>& f)
{
	...
	std::for_each(QSPEC_EXEC_PAR indexes.begin(), indexes.end(),
		[this, results, rho, qk, k, i, f](size_t index)
		{
				...
				for (size_t _i : i)
				{
					for (size_t index_ki = 0; index_ki < k.size(); ++index_ki)
					{
						...
						if (rho.at(index_rho).outerSize() == 1)
						{
							...
							// printf("y0: %s\n", fmt::format("{:.3e} + i{:.3e}", y.real(), y.imag()).c_str());
						...
						else
						{
							for (size_t _j : i)
							{
								for (size_t index_kj = 0; index_kj < k.size(); ++index_kj)
								{
									...
									// if (index_rho == 201) printf("rho: %s\n", fmt::format("({}, {}): {:.3e} + {:.3e}i", _i, _j, rho.at(index_rho)(_j, _i).real(), rho.at(index_rho)(_j, _i).imag()).c_str());
									// printf("y1: %s\n", fmt::format("{:.3e} + i{:.3e}", y.real(), y.imag()).c_str());
			...
			// printf("%s\n", fmt::format("r{}: {:.3e} + {:.3e}i", index_rho, _result.real(), _result.imag()).c_str());
...
size_t Atom::get_min_k(size_t i, size_t j)
{
	...
	else
	{
		if (ne == 1 && s0.get_s().at(0) == 0.5)
		{
			...
			return std::max(static_cast<size_t>(1), dl);
		}
		else
		{
			if (ds == 0)
