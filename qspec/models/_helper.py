...
from qspec.qtypes import array_like, ndarray, scalar_nd
...
def gen_model(
    ...
    if shape is None or not shape:
        ...
        if shape[0].islower():
            ...
            shape = eval(f"_spectrum.{shape}", {"_spectrum": _spectrum})
        ...
        if isinstance(convolve, str):
            if convolve[0].islower():
                ...
                if convolve not in _convolved.CONVOLVE:
                    raise ValueError(
                        f"The convolution {convolve} is not available. Choose one of {_convolved.CONVOLVE}"
                    )
                convolve = eval(f"_convolved.{convolve}Convolved", {"_convolved": _convolved})
    ...
    if hf_mixing:
        npeaks_model = _splitter.SplitterSummed(
            [spl(shape(), i, j_l, j_u, f"HF{n}", hf_config) for n, (i, j_l, j_u) in enumerate(ijj)]
        )
    else:
        npeaks_model = _splitter.SplitterSummed(
            [spl(shape(), i, j_l, j_u, f"HF{n}") for n, (i, j_l, j_u) in enumerate(ijj)]
        )
...
def find_models(
    model: _base.Model, sub_model: Union[_base.Model, type], model_list: Iterable = None
) -> list[_base.Model]:
    """
    :param model: The model to search in.
    :param sub_model: The submodel to find.
