...
def t_rank_k(k, spin_set):
    if k == 1:
        return spin_set
    if k > 1:
        t0 = t_rank_k(k - 1, spin_set)
        return {q: sy.Add(*[wi.clebsch_gordan(k - 1, 1, k, m0, m1, q)
                            * t0[m0] * spin_set[m1]
                            for m0 in range(-k + 1, k)
                            for m1 in range(-1, 2) if m0 + m1 == q])
...
def act_ops_im_jm(ops, i, j, mi, mj):
    coef = sy.S(1)
    _mi = mi
    _mj = mj
...
def act_sum_ops_im_jm(ops, i, j, mi, mj):
    return sy.Add(*[act_ops_im_jm(op, i, mi, j, mj) for op in ops])


def matrix_jm(op, i, j, mi0, mi1, mj0, mj1):
    # print(f'Input: {op}')
    ops = word_to_list(op)
...
def convert_to_python(s):
    return (str(s).replace('m_I', 'mi1').replace('m_J', 'mj1').replace('I', 'i').replace('J', 'j')
            .replace('*', ' * ').replace(' *  * ', ' ** ').replace('/', ' / ').replace('sqrt', 'np.sqrt'))
...
if __name__ == '__main__':
    ...
    for _k, _h in enumerate(hfs):
        ...
        print(f'mi0 == mi1     and mj0 == mj1    :'
              f' {convert_to_python(matrix_jm(_h, I, J, MI, MI, MJ, MJ))}')

        for __k in range(1, _k + 2):
            print(f'mi0 == mi1 + {__k} and mj0 == mj1 - {__k}:'
